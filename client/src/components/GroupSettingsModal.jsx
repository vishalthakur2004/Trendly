...
import { X, Upload, Globe, Lock, EyeOff, Users, Image as ImageIcon, Trash2 } from 'lucide-react';
import toast from 'react-hot-toast';
import { updateGroup, deleteGroup } from '../features/groups/groupsSlice';
...
const GroupSettingsModal = ({ group, onClose, onUpdate }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        cover_photo: null,
        settings: {
            allow_member_posts: group.settings?.allow_member_posts ?? true,
            allow_member_invites: group.settings?.allow_member_invites ?? false,
            post_approval_required: group.settings?.post_approval_required ?? false,
            allow_member_calls: group.settings?.allow_member_calls ?? true,
            allow_file_sharing: group.settings?.allow_file_sharing ?? true,
            max_members: group.settings?.max_members ?? 1000
        }
    });
    
    const [profilePreview, setProfilePreview] = useState(group.profile_picture || null);
    ...
    const handleDeleteGroup = async () => {
        try {
            const token = await getToken();
            await dispatch(deleteGroup({ groupId: group._id, token })).unwrap();
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl max-w-3xl w-full max-h-[90vh] overflow-hidden">
                ...
                <div className="border-b border-gray-200">
                    <nav className="flex">
                        {['general', 'permissions', 'danger'].map((tab) => (
                            <button
                                key={tab}
                ...
                <div className="p-6 max-h-96 overflow-y-auto">
                    ...
                    {activeTab === 'permissions' && (
                        <div className="space-y-6">
                            <div>
                                <h3 className="font-medium text-gray-900 mb-4">Group Permissions</h3>
                                <div className="space-y-4">
                                    <label className="flex items-center gap-3">
                                        ...
                                        />
                                        <div>
                                            <span className="font-medium">Allow member posts</span>
                                            <p className="text-sm text-gray-500">Members can create posts in this group</p>
                                        </div>
                                    </label>
                                    ...
                                    <label className="flex items-center gap-3">
                                        ...
                                        />
                                        <div>
                                            <span className="font-medium">Allow member invites</span>
                                            <p className="text-sm text-gray-500">Members can invite others to join the group</p>
                                        </div>
                                    </label>
                                    ...
                                    <label className="flex items-center gap-3">
                                        ...
                                        />
                                        <div>
                                            <span className="font-medium">Require post approval</span>
                                            <p className="text-sm text-gray-500">All posts must be approved by admins or moderators</p>
                                        </div>
                                    </label>
                                    ...
                                    <label className="flex items-center gap-3">
                                        ...
                                        />
                                        <div>
                                            <span className="font-medium">Allow group calls</span>
                                            <p className="text-sm text-gray-500">Members can start voice and video calls</p>
                                        </div>
                                    </label>
                                    ...
                                    <label className="flex items-center gap-3">
                                        <input
                                            type="checkbox"
                                            name="settings.allow_file_sharing"
                                            checked={formData.settings.allow_file_sharing}
                                            onChange={handleInputChange}
                                            className="rounded"
                                        />
                                        <div>
                                            <span className="font-medium">Allow file sharing</span>
                                            <p className="text-sm text-gray-500">Members can share files in messages</p>
                                        </div>
                                    </label>
                                </div>
                            </div>
                        ...
                        <div className="space-y-6">
                            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                                ...
                                <p className="text-sm text-red-700 mb-4">
                                    This action cannot be undone. All group data including posts, messages, and member information will be permanently deleted.
                                </p>
